...
/**
  * 自定义数据源实现SourceFunction类,非并行
  * @Author zhenxin.ma
  * @Date 2020/3/3 18:30
  * @Version 1.0
  */
//SourceFunction是非并行化的source所以不能指定并行度
class SourceFunctionWordCount extends SourceFunction[String] {
  var num = 0
  var isCancel = true
  //调用run方法向下游产生数据
...
object SourceFunctionWordCount {
  def main(args: Array[String]): Unit = {
    ...
    // 使用自定义的source
    val text: DataStream[String] = env.addSource(new SourceFunctionWordCount)
...
//    val wordCount: DataStream[(String, Int)] = keyStream.sum(1)
    ...
    env.execute("SourceFunctionWordCount")
  }
}
